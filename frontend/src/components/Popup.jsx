...
const Popup = ({ text, explanation, translation, audioText, audioExplanation, onClose }) => {
  ...
  const handlePlayAudio = (audioRef) => {
    ...
    if (audioExplanationRef.current && audioExplanationRef.current !== audioRef.current) {
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white p-8 rounded-lg shadow-lg relative max-w-4xl w-4/5 mx-auto" onClick={(e) => e.stopPropagation()}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
          ...
          <div className="border-4 border-gray-700 p-4 rounded-lg">
            <p className="text-xl font-bold">{isTranslated ? translation : explanation}</p>
            <audio
              controls
