  ...
  return (
    <div className="flex flex-col md:flex-row h-screen">
      <div className="flex-1 flex flex-col items-center justify-center bg-white">
          <img src="src\Images\Login.png" alt="eSharirbook Illustration" className="h-full w-full object-fill" />
      </div>
      <div className="flex-1 flex flex-col items-center justify-center bg-white p-6 md:p-10">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-xs md:max-w-sm">
          ...
          <div className="mb-6">
            <button type="submit" className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-teal-600 hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500">Sign in</button>
          </div>
        </form>
        <div className="text-center">
          <a href="/forgot-password" className="text-teal-500 text-sm hover:underline">Forgot password?</a>
          <p className="mt-4 text-sm">New to eSharirbook? <a onClick={() => {
        navigate("/signup")
    }} className="text-teal-500 cursor-pointer hover:underline">Create Account</a></p>
        </div>
      </div>
    </div>
  );
